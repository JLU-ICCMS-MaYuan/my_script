...
# Changer: Xuecheng Shao
#----------------------------------------------------------------------
# Because I add and remove many functional, this script only work for me.
#----------------------------------------------------------------------
#
...
try:
    ...
    print 'ERROR: please install libpyspg.'
    exit(0)
try:
    import numpy as np
except:
                ...
                if 'Optimized' in date[j]:
                    for k in range(j+1, ldate):
                        ...
                        if 'Ele_Num' in date[k]:
                            typt = map(int, date[k].split('=')[1].split())
                            natom = sum(typt)
                            #print typt,natom
                ...
                if 'Optimized' in date[j]:
                    ...
                    while lcurr :
                        ...
                        if 'Energy' in line:
                            ...
                            typt = map(int, line.split('=')[1].split())
                            natom = sum(typt)
        ...
        if 'nstruct' in date[i]:
            ...
            while lcurr :
                ...
                if 'Ele_Num' in date[j] :
                    typt = map(int, date[j].split('=')[1].split())
                    typt = gcdnum(typt)
...
def parseOpt(optf='opt', hm=False):
    ...
    for optfile in ofls:
        ...
        for i in range(0, nstr):
            ...
            lat = []
            for i in range(2, 5):
                lat.append(map(float, strutmp[i].split()))
            lat = np.array(lat, float)
            typt = map(int, strutmp[5].split())
            pos = []
...
def vsckit(structure, vsce, name_ele, options, prec_pool, is_refine, is_prim, hard, cl, norefine, hm, bg, xrd):
    ...
    typt_dict = {}
    BStruct = []
    for i in xrange(len(typt_list)):
        str1 = '_'.join(map(str, typt_list[i]))
        typt_dict[str1] = i
        BStruct.append([])
    # print BStruct
    for x in Bstruct:
...
def run():
    ...
    if options.mprec is None:
        ...
        try:
            prec_pool = map(float, options.mprec.split())
        except:
