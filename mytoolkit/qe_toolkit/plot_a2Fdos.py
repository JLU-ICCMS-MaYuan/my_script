...
def get_eliashberg_Tc(eliashberg_x_path, idx):
    ...
    os.system(f"sed '1,5d' "+ "  a2F.dos"+str(idx) + "  | sed '/lambda/d' | awk '{print $1/2, $2}' > ALPHA2F.OUT")
    os.system(f"{eliashberg_x_path} > eliashberg.log")
    eliashberg_gap_t_out = Path.cwd().joinpath("ELIASHBERG_GAP_T.OUT")
    process_eliashberg_gap_t_out = Path.cwd().joinpath("process_ELIASHBERG_GAP_T.OUT")
    os.system(f"sed '/E/!d' {eliashberg_gap_t_out} > {process_eliashberg_gap_t_out}")
