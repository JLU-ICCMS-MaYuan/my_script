...
class qe_submitjob:
    ...
    def __init__(self, submit_path, submit_job_system="slurm", running_mode=None, *args, **kwargs):
    ...
    def slurm_job_system(self):
        if self.running_mode == "relax":
            self.slurmrelax(self.submit_path)
        if self.running_mode == "scffit":
            self.slurmscfFit(self.submit_path)
        if self.running_mode == "scf":
            self.slurmscf(self.submit_path)
        if self.running_mode =="ph_no_split":
            self.slurmph_no_split(self.submit_path)
        if self.running_mode =="ph_split_form_dyn0":
            self.slurmph_split_form_dyn0(self.submit_path)
        if self.running_mode =="ph_split_set_startlast_q":
            self.slurmph_split_set_startlast_q(self.submit_path, self.system_name)
        if self.running_mode =="q2r":
            self.slurmq2r(self.submit_path)
        if self.running_mode =="matdyn":
            self.slurmmatgen(self.submit_path)
        if self.running_mode =="matdyn_dos":
            self.slurmmatgen_dos(self.submit_path)
        if self.running_mode =="lambda":
            self.slurmlambda(self.submit_path)

    def pbs_job_system(self):
        if self.running_mode == "relax":
            self.pbsrelax(self.submit_path)
        if self.running_mode == "scffit":
    ...
    def pbsrelax(self, pbs_dirpath):
        ...
        with open(pbs_script_filepath, "w") as pbs:
            ...
            pbs.write('\n\n                                                                     \n')
            pbs.write('mpirun -np 28 /public/home/mayuan/software/qe-7.1/bin/pw.x -npool 4 <scf.in> scf.out \n')
