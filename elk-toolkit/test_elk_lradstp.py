...
def write_elk_in(lradstp, elk_in, work_path):
   
    with open(elk_in, 'r+') as f:
        lines = f.readlines()
        # 寻找ngridk关键字并修改下一行的值
        for j in range(len(lines)):
            if 'lradstp' in lines[j].lower():
                lines[j + 1] = f'{lradstp}\n'
...
if __name__ == "__main__":
    ...
    print("    python test_vasp_lradstp.py  1.0 10.0  1.0")
    print("    python test_vasp_lradstp.py 10.0  1.0 -1.0")
    print("    以上两种都是产生1到10, 间隔为1的lradstp")
    lradstps = list(np.round(np.arange(eval(start), eval(stop), eval(step)), decimals=2))
#    except:
...
#        kresolutions = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.19, 0.18, 0.17, 0.16, 0.15, 0.14, 0.13, 0.12, 0.1, 0.05]
    ...
    print("        for i in {}; do cd $i; qsub submit.sh;   cd ..; done".format(' '.join(list(map(str, lradstps)))))
    print("        for i in {}; do cd $i; sbatch submit.sh; cd ..; done".format(' '.join(list(map(str, lradstps)))))
    ...
    for lradstp in lradstps:
        test_path = os.path.abspath(str(lradstp))
        if not os.path.exists(test_path):
            os.mkdir(test_path)
        write_elk_in(lradstp, elk_in_path, test_path)
    ...
    lradstp_dE_dF_dV = []
    for lradstp in lradstps:
        totenergy_out_path = os.path.join(str(lradstp), "TOTENERGY.OUT")
        # dE_per_atom, force, virial_per_atom = get_info_per_atom(atom, totenergy_out_path)
        # lradstp_dE_dF_dV.append([lradstp, dE_per_atom, force, virial_per_atom])
        dE_per_atom = get_info_per_atom(atom, totenergy_out_path)
        lradstp_dE_dF_dV.append([lradstp, dE_per_atom])
    lradstp_dE_dF_dV = sorted(lradstp_dE_dF_dV, key=lambda x: x[0],  reverse=False)

    # 方式一：通过相邻两个lradstp的能量的差判断收敛性
    # Ediff = np.diff(lradstp_dE, axis=0)[:,-1]
    # Ediff = np.insert(Ediff, 0, [0.0])
    # Ediff = Ediff[:, np.newaxis]
    # lradstp_dE_Hdiff = np.hstack((lradstp_dE, Ediff))

    # 方式二：所有kpoints的能量减去最密lradstp的能量判断收敛性

    with open("lradstp_dE.csv", 'w') as f:
        f.write("{},{},{},{}\n".format("lradstp", "Ediff(meV/atom)", "Fdiff(meV/A)", "Vdiff(meV/atom)"))
        print("{:<12},{:<14},{:<14},{:<14}".format("lradstp", "Ediff(meV/atom)", "Fdiff(meV/A)", "Vdiff(meV/atom)"))
        for idx, kefv in enumerate(lradstp_dE_dF_dV):
            kmeshes = kefv[0] 
            delta_E = kefv[1] - lradstp_dE_dF_dV[-1][1]
            # delta_F = kefv[2] - lradstp_dE_dF_dV[-1][2]
            # delta_V = kefv[3] - lradstp_dE_dF_dV[-1][3]
            ...
            print("If all elk are OK, lradstp_dE.csv will be wroten in current position")

    # 绘制图表
    df = pd.read_csv("lradstp_dE.csv")
    plt.figure(figsize=(10, 6))  # 设置图形大小
    plt.plot(df['lradstp'], df['Ediff(meV/atom)'], marker='o')  # 绘制折线图
    plt.title('lradstp vs Total Energy')  # 设置图表标题
    plt.xlabel('lradstp')  # 设置x轴标签
    plt.ylabel('Total Energy (Hartree)')  # 设置y轴标签
    plt.grid(True)  # 显示网格
    plt.savefig('elk_lradstp_conv.png')  # 保存图形
    plt.show()  # 显示图形
