...
def write_lobsterin(dirname, mode=5, COHPstartEnergy=None, COHPendEnergy=None, species_custom1=None, species_custom2=None, lower_d=None, upper_d=None, zval_dict=None, custom_basis_dict=None):
    ...
    with open(lobsterin_path, "w") as f:
        ...
        for element, orbitals in zval_dict.items():
            f.write('basisfunctions {} {}\n'.format(element, orbitals)) # 基组（直接使用根据vasp拟合的基组以及默认的基函数）
...
def main():
    ...
    # 写入LOBSTER输入文件
    write_lobsterin(dirs, mode=mode, COHPstartEnergy=COHPstartEnergy, COHPendEnergy=COHPendEnergy, species_custom1=species_custom1, species_custom2=species_custom2, lower_d=lower_d, upper_d=upper_d, zval_dict=zval_custom, custom_basis_dict=custom_basis)
