...
def parse_arguments():
    ...
    parser.add_argument('-i', '--input-dir',          type=str, required=True, help='Directory or pattern containing the structure files.')
    ...
    parser.add_argument('-t', '--file-types',          type=str, nargs='+', default=['POSCAR*'], help='List of type of file types, default=[POSCAR*]')
    return parser.parse_args()


def search_files(input_dir, file_types, detailed_conditions):
    # 搜索所有匹配的文件
    ...
    for file_type in file_types:
        # print(glob.glob(os.path.join(input_dir, '**', file_type), recursive=True))
...
def main():
    ...
    dst_files = search_files(input_dir, file_types, detailed_conditions)
