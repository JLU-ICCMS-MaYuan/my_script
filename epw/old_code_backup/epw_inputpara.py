...
class epw_inputpara(epw_base):
    ...
    def __init__(
        ...
        **kwargs: dict, # 这里非常重要, 因为 epw_inputpara 的__init__需要读入kwargs, 其它需要继承这个类的子类也需要保有这个参数kwargs
        ...
        if not hasattr(self, "keep_a2f"):
            self.keep_a2f = True
        else:
            self.keep_a2f = eval(self.keep_a2f)
        logger.info(f'keep_a2f = {self.keep_a2f}, True means a2f will be reserved and do not link files generated by epw_elph, False means the a2f will be delete and link files generated by epw_elph.') 
