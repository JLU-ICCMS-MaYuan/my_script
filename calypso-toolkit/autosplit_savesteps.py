...
class Autocalypso(object):
    ...
    def backupfiles(self):
        ...
        with open("step", "r") as s:
            line = s.read()
        step_num = int(line.strip("\n"))
        # 备份当前代计算好的POSCAR_*, CONTCAR_*, OUTCAR_*
        new_backup_dir = "step_"+str(step_num-1)
        ...
        for i in range(self.StrNum):
            if os.path.exists(r'POSCAR_%s' % (str(i+1))):
                os.system('cp POSCAR_%s  %s/POSCAR_%s' % (str(i+1),new_backup_dir,str(i+1)))
                print("finish backup POSCAR_%s" % (str(i+1)))
            if os.path.exists(r'CONTCAR_%s' % str(i+1)):
                os.system('cp CONTCAR_%s  %s/CONTCAR_%s' % (str(i+1),new_backup_dir,str(i+1)))
                print("finish backup CONTCAR_%s" % (str(i+1)))
            if os.path.exists(r'OUTCAR_%s' % str(i+1)):
                os.system('cp OUTCAR_%s  %s/OUTCAR_%s' % (str(i+1),new_backup_dir,str(i+1)))
    ...
    def control_vasp(self):
        ...
        for i in range(self.StrNum):
            if os.path.exists(r'./%s/POSCAR_%s' % (str(i+1), str(i+1))):
                os.system('cp %s/POSCAR_%s  POSCAR_%s' % (str(i+1),str(i+1),str(i+1)))
            else:
                print(i + 1,'th VASP JOB WRONG NO POSCAR')
