...
class split_wyckoffs_multprocessing:
    '''
    '''
    def __init__(
        self              ,
    ...
    def get_group(
        ...
        occupied_number_ranges,
            ...
            if composition_json.exists():
                ...
                print("\nNote:-----------------------")
                print("    The composition.json file doesn't exist, the composition.json will be generated by multi-processing !!!!")
                _group = {} 
                with ProcessPoolExecutor(max_workers=self.max_workers) as pool:
                    futures = [
                        pool.submit(
                            ...
                            satisfied_spg,
                            wyckoffpositions,
                            ) for satisfied_spg in satisfied_spgs]
                for future in as_completed(futures):
                    # 合并不同字典中相同的键对应的值
                    for formula, value in future.result().items():
                        if formula in _group:
                            _group[formula].extend(value)
                        else:
                            _group[formula] = value
    ...
    def mult_hydirdes(
        ...
        wps_bool = wyckoffpositions[satisfied_spg]
        ...
        for wps in product(*total_ele_wps):
            if self.check_duplicate(wps, wps_bool):
                ...
                n = append_composition(_group, self.nameofatoms, wps, satisfied_spg, self.numberofatoms, nelems, self.hydrogen_content)
    ...
    def init_from_config(cls, config_d: dict):
        self = cls(
            ...
            max_workers=config_d['max_workers'],
        )
        return self
...
if __name__ == "__main__":
    ...
    # binary
    # binary
    spl_wps = split_wyckoffs_multprocessing(
        work_path='.',
        spacegroup_number = [[218, 230]],
        nameofatoms = ["Ca", "H"],
        numberofatoms = [1, 6],
        mults_ranges=[[1,2],
                      [1,12]],
