FUNCTION Cal_Volume(matrix)
    ...
    real(dp), external    :: DetInMath
    real(DP) :: Cal_Volume

    Cal_Volume = abs(DetInMath(matrix))
...
FUNCTION DetInMath(matrix)
   !#! Xin Yu edit det to detInMath
    USE kinds, ONLY: DP, i4b
    ...
    DetInMath = Matrix(1, 1)*(Matrix(2, 2)*Matrix(3, 3) - Matrix(2, 3)*Matrix(3, 2)) &
          - Matrix(1, 2)*(Matrix(2, 1)*Matrix(3, 3) - Matrix(2, 3)*Matrix(3, 1)) &
          + Matrix(1, 3)*(Matrix(2, 1)*Matrix(3, 2) - Matrix(3, 1)*Matrix(2, 2))

END FUNCTION DetInMath
