...
contains
    subroutine RandomSeedSet(arg1)
        ! variables declaration
        INTEGER(4), OPTIONAL, intent(in) :: arg1
        INTEGER(4) :: i_seed
        INTEGER(4), DIMENSION(:), ALLOCATABLE :: a_seed
        INTEGER(4)::index1
        if (present(arg1)) then
            ...
            do index1 = 1, SIZE(a_seed, 1)
                a_seed(index1) = arg1*index1
            end do
            CALL RANDOM_SEED(put=a_seed)
            DEALLOCATE (a_seed)
            ! ----- Done setting up random seed -----
        end if
    end subroutine RandomSeedSet
