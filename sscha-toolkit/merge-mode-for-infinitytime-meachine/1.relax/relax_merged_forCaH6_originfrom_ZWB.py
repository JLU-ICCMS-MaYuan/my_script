...
atom_masses = {"Ca": 40.078,
               "H" : 1.00794}
bool_value = True
pseudo = {"Ca": "Ca_pbe_v1.uspp.F.UPF",
          "H" : "H.pbe-rrkjus_psl.1.0.0.UPF"}
input_data = {
               'control' : {
                   'disk_io' : 'none',
                   'pseudo_dir' : '/public/home/renlanxuecssc/work/H44/pseudo',
                   'tstress' : bool_value,
                   'tprnfor ': bool_value
                           },
               'system' : {
...
k_points = (2,2,4) # The k points grid (you can alternatively specify a kspacing)
...
my_hpc = sscha.Cluster.Cluster(mpi_cmd=r"mpirun -n 96", AlreadyInCluster=True) # AlreadyInCluster=True代表本地提交任务, 既可以把当前python脚本提交到节点上, 也可以在主节点运行当前脚本 AlreadyInCluster=False代表通过ssh连接远程机器提交任务
my_hpc.workdir = "/public/home/renlanxuecssc/work/H45/200/run"
my_hpc.binary = "/public/home/renlanxuecssc/software/qe-7.2/bin/pw.x -npool NPOOL -i  PREFIX.pwi > PREFIX.pwo"
...
"""
my_hpc.n_cpu = 96  # We will use 32 processors
my_hpc.n_nodes = 1 # In 1 node
my_hpc.n_pool = 1  # This is an espresso specific tool, the parallel CPU are divided in 4 pools
my_hpc.batch_size = 8     # We can also choose in how many batch of jobs we want to submit simultaneously
my_hpc.job_number = 25    # and how many configurations for each job. # In this way we submit 25 jobs, each one with 8 configurations (overall 200 configuration at time)
my_hpc.set_timeout(600)   # We give 25 seconds of timeout
my_hpc.time = "100:00:00" #  We can specify the time limit for each job, 5 minutes
...
max_population = 20 # If sscha calculation is executed to dyn_pop20_, so the calculation will be stopped.
target_press = 200
