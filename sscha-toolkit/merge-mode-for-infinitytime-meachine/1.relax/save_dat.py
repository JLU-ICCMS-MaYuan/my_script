...
FORCE_NAME=f"forces_population{END_POP_IDX}"
STRESS_NAME=f"pressures_population{END_POP_IDX}"
ENER_NAME=f"energies_supercell_population{END_POP_IDX}.dat"
...
for file in output_files:
    ...
    force_file = os.path.join("temp_ensembles_dat", FORCE_NAME+f"_{id_number}.dat")
    stress_file = os.path.join("temp_ensembles_dat", STRESS_NAME+f"_{id_number}.dat")
    np.savetxt(force_file, forces)
    np.savetxt(stress_file, stress)
    
# Now we read all the configurations, we can save the energy file
energy_file = os.path.join("temp_ensembles_dat", ENER_NAME)
