...
class Cluster(object):
    ...
    def __init__(self, hostname=None, pwd=None, extra_options="", workdir = "",
                 ...
                 mpi_cmd=r"srun --mpi=pmi2 -n NPROC",AlreadyInCluster=False):
        """
        SETUP THE CLUSTER
        =================
        ...
        self.pwd = pwd
        self.AlreadyInCluster = AlreadyInCluster
        self.sshcmd = "ssh"
...
#            
            ...
            if self.AlreadyInCluster:
                cmd = self.scpcmd + " %s %s/" % (tar_file, self.workdir)
...
""".format(list_of_input))
            ...
            if self.AlreadyInCluster:
                cmd = self.scpcmd + "%s %s/" % (os.path.join(self.workdir, tar_name), self.local_workdir)
            cp_res = self.ExecuteCMD(cmd, False)
            if not cp_res:
                print ("Error while executing:", cmd)
                print ("Return code:", cp_res)
...
""".format(list_of_input))
    ...
    def submit(self, script_location):
        ...
        if self.AlreadyInCluster:
            cmd = "{submit_cmd} {script}".format(submit_cmd = self.submit_command, script = script_location) 
...
#        
        ...
        if self.AlreadyInCluster:
            cmd = self.scpcmd + " %s/%s.sh %s" % (self.local_workdir, label, self.workdir)
        self.ExecuteCMD(cmd, False)
#        cp_res = os.system(cmd)
#        if cp_res != 0:
#            print "Error while executing:", cmd
...
#            
        ...
        if self.AlreadyInCluster:
            cmd = self.scpcmd + " %s/%s%s %s" % (self.local_workdir, label, in_extension, self.workdir)
        cp_res = self.ExecuteCMD(cmd, False)
        #cp_res = os.system(cmd)
        if not cp_res:
            #print "Error while executing:", cmd
...
#            sys.stderr.write(cmd + ": exit with code " + str(cp_res))
        ...
        cmd = self.scpcmd + " %s:%s/%s%s %s/" % (self.hostname, self.workdir, label, out_extension, 
                                                 self.local_workdir)
        if self.AlreadyInCluster:
            cmd = self.scpcmd + " %s/%s%s %s/" % (self.workdir, label, out_extension,
                                                 self.local_workdir)
        cp_res = self.ExecuteCMD(cmd, False)
        #cp_res = os.system(cmd)
        if not cp_res:
...
#            print workdir
    ...
    def parse_string(self, string):
        ...
        #print cmd
        if self.AlreadyInCluster:
            cmd = "echo {}".format(parse_symbols(string))
