...
outcars = [f'{str(i)}/OUTCAR' for i in range(400, 1200, 50)]
free_per_atom = []
virial_per_atom = []
Tforce = []
...
print("{:<10} {:<10} {:<10} {:<10}".format('encut(eV)', 'free_energy(meV)','force(meV/A)','virial'))
for idx, ff in enumerate(free_per_atom):
    ...
    print("{:<10} {:<10.6f} {:<10.6f} {:<10.6f}".format(outcars[idx].split('_')[-1], np.nanmax(detla_fe)*1000, np.nanmax(detla_force)*1000, np.nanmax(detla_vir)*1000))
