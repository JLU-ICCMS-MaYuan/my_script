...
j=0;x=1; for i in `squeue | awk '{print $1}'`; do  let j+=x; done; echo $j
    ...
    qe_main.py -i ./relax.out -w ./ -p 压力值  -j pbs phono -m mode=q2r        core=1 npool=1 queue=local
    qe_main.py -i ./relax.out -w ./ -p 压力值  -j pbs phono -m mode=matdyn     core=1 npool=1 queue=local qinserted=50
    qe_main.py -i ./relax.out -w ./ -p 压力值  -j pbs dos   -m mode=matdyn_dos core=1 npool=1 queue=local qpoints='8 8 8' ndos=500 

    qe_main.py -i ./relax.out -w ./ -p 压力值  -j pbs sc    -m mode=McAD       core=1 npool=1 queue=local top_freq=80 deguass=0.5 screen_constant=0.1
    qe_main.py -i ./relax.out -w ./ -p 压力值  -j pbs sc    -m mode=eliashberg core=1 npool=1 queue=local temperature_points=10000 a2F_dos=a2F.dos3
