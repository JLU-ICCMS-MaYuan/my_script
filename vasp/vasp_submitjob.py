...
class vasp_submitjob:
    ...
    def submit_mode1(self, jobname):
        ...
        for input_name in inputfilename:
            input_file = Path(self.sub_workpath).joinpath(input_name)
            if not input_file.exists():
                raise FileExistsError(f" {input_name} doesn't exist")
                
        job_file = Path(self.sub_workpath).joinpath(jobname)
    ...
    def submit_mode2(self, jobname):
        ...
        poscar_files = os.listdir(self.sub_workpath)
        poscar_number_list = [patter.match(x).group() for x in poscar_files if patter.match(x)]
        for poscar_number in poscar_number_list:
            dst_number_dir = Path(self.sub_workpath).joinpath("disp-" + poscar_number.split("-")[-1])
            if not os.path.exists(dst_number_dir):
                os.makedirs(dst_number_dir)
            src_poscar = Path(self.sub_workpath).joinpath(poscar_number) ; dst_poscar = Path(dst_number_dir).joinpath("POSCAR"); shutil.copy(src_poscar, dst_poscar)
            src_potcar = Path(self.sub_workpath).joinpath("POTCAR")      ; dst_potcar = Path(dst_number_dir).joinpath("POTCAR"); shutil.copy(src_potcar, dst_potcar)
            src_incar  = Path(self.sub_workpath).joinpath("INCAR")       ; dst_incar  = Path(dst_number_dir).joinpath("INCAR" ); shutil.copy(src_incar, dst_incar )
            src_kpoints= Path(self.sub_workpath).joinpath("KPOINTS")     ; dst_kpoints= Path(dst_number_dir).joinpath("KPOINTS"); shutil.copy(src_kpoints, dst_kpoints)
            src_submit = Path(self.sub_workpath).joinpath(jobname)       ; dst_submit = Path(dst_number_dir).joinpath(jobname);  shutil.copy(src_submit, dst_submit)
            cwd = src_poscar.cwd()
