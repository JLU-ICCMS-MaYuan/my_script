...
The Program Logic:
    1. 在当前目录下准备好POSCAR
    2. 指定结构弛豫的外部压强, 读取POSCAR, 读取赝势目录
    3. 写qe进行结构弛豫的输入文件rx.vc.in
Use examples:
    vasp2qe_relax.py -ppd pp -p 1000
        -ppd pp         指定赝势目录
        -p 1000         指定压强 1000 Kbar
'''
import os
import re
...
if __name__ == "__main__":
    ...
    with open("rx.vc.in", "w") as qe:
        qe.write("&CONTROL\n")
        qe.write(" calculation='vc-relax',            \n")  # 这里有relax, md, vc-relax, vc-md 四种计算模式
        ...
        qe.write("cell_factor = 2.0,               \n")
        qe.write("press = {},                      \n".format(pressure))  # 单位是：
        qe.write("press_conv_thr = 0.1,            \n")
        qe.write("/\n")
