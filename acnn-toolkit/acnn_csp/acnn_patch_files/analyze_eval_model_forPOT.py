...
if __name__ == '__main__':
    ...
    energy_df = pd.DataFrame({
        "energy_DFT": e / natoms,
        "energy_NN": e_pred / natoms
    })
    energy_csv = f"energy_data_{log_file.split('.')[0]}.csv"
    energy_df.to_csv(energy_csv, index=False)
    print(f"能量数据已保存到：{energy_csv}")
    
    # ---------- 2. 保存力数据（长度一致） ----------
    force_df = pd.DataFrame({
        "force_DFT": f.flatten(),
        "force_NN": f_pred.flatten()
    })
    force_csv = f"force_data_{log_file.split('.')[0]}.csv"
    force_df.to_csv(force_csv, index=False)
    print(f"力数据已保存到：{force_csv}")
    
    # ---------- 3. 保存应力数据（长度一致） ----------
    stress_df = pd.DataFrame({
        "stress_DFT": v.flatten(),
        "stress_NN": v_pred.flatten()
    })
    stress_csv = f"stress_data_{log_file.split('.')[0]}.csv"
    stress_df.to_csv(stress_csv, index=False)
    print(f"应力数据已保存到：{stress_csv}")
